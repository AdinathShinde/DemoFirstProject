...
{
    public class SqlSP_DeleteCareRepository : ISP_DeleteCaretRepository
    {
        private readonly ApplicatioDbContext dbContext;
        public SqlSP_DeleteCareRepository (ApplicatioDbContext dbContext)
        {
            this.dbContext = dbContext;
        }
        public async Task<List<ApiRespones>> getasync(SP_DeleteCaret input)
        {
            List<ApiRespones> result = new List<ApiRespones>();
            using (var command = dbContext.Database.GetDbConnection().CreateCommand())
            {
                command.CommandText = "[SP_DeleteCaret]";
