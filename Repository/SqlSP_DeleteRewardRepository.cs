...
{
    public class SqlSP_DeleteRewardRepository : ISP_DeleteRewardRepository
    {
        private readonly ApplicatioDbContext dbContext;
        public SqlSP_DeleteRewardRepository (ApplicatioDbContext dbContext)
        {
            this.dbContext = dbContext;
        }
        public async Task<List<ApiRespones>> getasync(SP_DeleteReward input)
        {
            List<ApiRespones> result = new List<ApiRespones>();
            using (var command = dbContext.Database.GetDbConnection().CreateCommand())
            {
                command.CommandText = "[SP_DeleteReward]";
