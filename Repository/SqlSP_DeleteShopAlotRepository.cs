...
{
    public class SqlSP_DeleteShopAlotRepository : ISP_DeleteShopAlotRepository
    {
        private readonly ApplicatioDbContext dbContext;
        public SqlSP_DeleteShopAlotRepository(ApplicatioDbContext dbContext)
        {
            this.dbContext = dbContext;
        }
        public async Task<List<ApiRespones>> getasync(SP_DeleteShopAlot input)
        {
            {
                List<ApiRespones> result = new List<ApiRespones>();
                using (var command = dbContext.Database.GetDbConnection().CreateCommand())
                {
                    command.CommandText = "[SP_DeleteShopAlot]";
                    command.CommandType = System.Data.CommandType.StoredProcedure;

                    var parameter = command.CreateParameter();
                    parameter.ParameterName = "@SHID";
                    parameter.Value = input.SHID;
